...
const invalidateAdminDashboardCache = async () => {
  try {
    await Promise.all([
      invalidateCacheByPrefix("admin:dashboard:v1"),
      invalidateCacheByPrefix("admin:dm-detail:v1"),
      invalidateCacheByPrefix("admin:platform-revenue-intelligence:v1"),
    ]);
  } catch (error) {
    console.error("Error invalidating admin dashboard cache:", error.message);
  }
};

const getAdminDashboard = async (req, res) => {
  try {
    const cacheKey = buildCacheKey("admin:dashboard:v1");
