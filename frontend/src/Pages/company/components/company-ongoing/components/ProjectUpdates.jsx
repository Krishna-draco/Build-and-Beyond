...
const ProjectUpdates = ({ project, expandedUpdates, onRefresh }) => {
  ...
  const makeFeeKey = (projectId, milestonePercentage) =>
    `${projectId}_${milestonePercentage}`;
    ...
    try {
      ...
      if (!orderResponse.ok || !orderResult.success) {
        throw new Error(
          orderResult.message ||
            orderResult.error ||
            "Failed to create platform fee order",
        );
      }
            ...
            if (!verifyResponse.ok || !verifyResult.success) {
              throw new Error(
                verifyResult.message ||
                  verifyResult.error ||
                  "Platform fee verification failed",
              );
            }
  ...
  const handlePlatformFeeTestMarkPaid = async (
    projectId,
    milestonePercentage,
  ) => {
    const key = makeFeeKey(projectId, milestonePercentage);
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result.message ||
            result.error ||
            "Failed to mark platform fee paid in test mode",
        );
      }
                  ...
                  <div className="ongoing-milestone-header">
                    <div>
                      <h4 className="ongoing-milestone-title">
                        {getPhaseForMilestone(project, milestone.percentage)
                          ?.name || `${milestone.percentage}% Milestone`}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="ongoing-milestone-message">
                        ...
                        <p>
                          <strong>Phase:</strong> {phase.name || "Phase"}
                        </p>
                        <p>
                        ...
                        >
                          ...
                          >
                            🚀 Released at phase start:{" "}
                            <strong>
                              ₹
                              {phase.amount
                                ? Math.round(
                                    Number(phase.amount) * 0.75,
                          ...
                          >
                            🔒 Held until customer approval:{" "}
                            <strong>
                              ₹
                              {phase.amount
                                ? Math.round(
                                    Number(phase.amount) * 0.25,
                                  ).toLocaleString("en-IN")
                                : "0"}
                            </strong>{" "}
                            (25%)
                          ...
                          >
                            🧾 Platform fee due after full payout:{" "}
                            <strong>
                              ₹
                              {phase.amount
                                ? Math.round(
                                    Number(phase.amount) * 0.05,
                                  ).toLocaleString("en-IN")
                                : "0"}
                            </strong>{" "}
                            (5%)
                          ...
                          phase.subdivisions.length > 0 && (
                            <div className="ongoing-conversation-messages">
                              ...
                              {phase.subdivisions.map((sub, sIdx) => (
                                <div
                                  key={sIdx}
                                  className="ongoing-conversation-msg msg-company"
                                >
                    ...
                    milestone.conversation.length > 0 && (
                      <div className="ongoing-conversation-history">
                        <strong className="conversation-title">
                          💬 Conversation History (
                          {milestone.conversation.length}{" "}
                          {milestone.conversation.length === 1
                            ? "message"
                            : "messages"}
                          )
                        </strong>
                  ...
                  {milestone.needsRevision && milestone.customerFeedback && (
                    <div className="ongoing-customer-feedback">
                      ...
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `../addnewproject?projectId=${project._id}&updateCheckpoint=${milestone.percentage}`,
                          )
                        }
                      >
                        📝 Respond & Update
                  ...
                  {(() => {
                    ...
                    const upfrontTarget = Number(
                      payout?.immediateReleaseAmount ||
                        (totalPhaseAmount > 0 ? totalPhaseAmount * 0.75 : 0),
                    );
                    const completionTarget = Math.max(
                      totalPhaseAmount - upfrontTarget,
                      0,
                    );
                    const completionPaidSoFar = Math.max(
                      paidSoFar - upfrontTarget,
                      0,
                    );

                    const isUpfrontPaid =
                      upfrontTarget > 0 && paidSoFar >= upfrontTarget;
                    const isCompletionPaid =
                      completionTarget > 0
                        ? payout?.status === "released" ||
                          completionPaidSoFar >= completionTarget
                        : false;

                    const fmt = (amount) =>
                      `Rs ${Number(amount || 0).toLocaleString("en-IN")}`;

                    const showPaymentBadges = Boolean(
                      payout && totalPhaseAmount > 0,
                    );
                    ...
                    const effectivePlatformFeeStatus =
                      ...
                      Number(payout?.platformFee || 0) > 0
                        ? payout?.platformFeeStatus === "not_due"
                          ? "pending"
                          : payout?.platformFeeStatus
                        : "not_due");
                    const effectivePlatformFeeAmount = Number(
                      summary?.platformFee ?? payout?.platformFee ?? 0,
                    );
                    const effectiveInvoiceUrl =
                      summary?.platformFeeInvoiceUrl ||
                      payout?.platformFeeInvoiceUrl ||
                      null;
                      ...
                      <>
                            ...
                            >
                              ...
                              <span
                                style={{
                                  ...
                                  padding: "5px 10px",
                                  borderRadius: "999px",
                                  background: isCompletionPaid
                                    ? "#dcfce7"
                                    : "#fef3c7",
                                  color: isCompletionPaid
                                    ? "#166534"
                                    : "#92400e",
                              ...
                              >
                                25% Payment:{" "}
                                {isCompletionPaid ? "Done" : "Pending"} (
                                {fmt(completionTarget)})
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        {["pending", "collected"].includes(
                          ...
                          >
                            ...
                            >
                              Pay platform fee through Razorpay and upload
                              invoice proof for records.
                            </p>
                            ...
                            {ENABLE_TEST_SKIP &&
                              effectivePlatformFeeStatus === "pending" && (
                                ...
                                >
                                  Test mode shortcut is enabled for this
                                  environment.
                                </p>
                              )}
                            {hasInvoice ? (
            ...
            <div className="ongoing-completion-section">
              ...
              <div className="ongoing-completion-images">
                {project.completionImages.map((img, idx) => (
                  <img
                    key={idx}
                    src={`${window.__APP_API_BASE_URL__ || import.meta.env.VITE_API_BASE_URL || "http://localhost:3000"}/${img}`}
          ...
          {project.customerReview?.rating && (
            <div className="ongoing-customer-review">
              ...
              <p className="review-date">
                ...
                {new Date(project.customerReview.reviewDate).toLocaleDateString(
                  "en-IN",
                  {
                    day: "numeric",
                    month: "long",
