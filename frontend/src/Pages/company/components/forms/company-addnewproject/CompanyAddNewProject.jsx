      ...
      try {
        ...
        if (found) {
          ...
          setTargetCompletionDate(
            found.targetCompletionDate
              ? new Date(found.targetCompletionDate).toISOString().split("T")[0]
              : "",
          );
          ...
          if (updateCheckpoint) {
            ...
            const milestone = found.milestones.find(
              (m) => m.percentage === checkpointNum && m.isCheckpoint,
            );
  ...
  const validate = () => {
    ...
    if (selectedMilestone) {
      ...
      if (isCheckpoint && !milestoneMessage.trim()) {
        v.push(
          `Please add a message for the ${milestone}% checkpoint so the customer can review and approve.`,
        );
      }
      ...
      if (maxAllowed === null) {
        v.push(
          "You cannot update progress until the previous checkpoint has been approved by the customer.",
        );
      } else if (milestone > maxAllowed) {
        v.push(
          `You can adjust between ${floor}% and ${maxAllowed}% only. Reach and get approval for ${maxAllowed}% to move further.`,
        );
      }
    }
    ...
    else if (new Date(targetCompletionDate) <= today)
      v.push("Target completion date must be in the future");
    if (!currentPhase) v.push("Current phase is required");
    if (mainImage && !validateFile(mainImage))
      v.push("Main file must be PDF/JPG/PNG and <= 5MB");
    for (let f of additionalImages)
      if (!validateFile(f)) {
        v.push("Each additional file must be PDF/JPG/PNG and <= 5MB");
        break;
      }
    if (updateText && updateText.length > 500)
      v.push("Update text cannot exceed 500 characters");
    if (updateImage && !validateFile(updateImage))
      v.push("Update image must be PDF/JPG/PNG and <= 5MB");
    setErrors(v);
    return v.length === 0;
  };
    ...
    try {
      ...
      if (selectedMilestone) {
        ...
        if (isCheckpoint && milestoneMessage.trim()) {
          ...
          formData.append(
            "milestoneMessage",
            `Progress update to ${selectedMilestone}%`,
          );
        }
      }
      ...
      const res = await fetch(
        `${window.__APP_API_BASE_URL__ || import.meta.env.VITE_API_BASE_URL || "http://localhost:3000"}/api/projects/update`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
