...
let adminDataInFlight = null;
let adminDataLastResult = null;
let adminDataLastAt = 0;
const ADMIN_DATA_DEDUPE_WINDOW_MS = 2000;

const fetchAdminDataPayload = async () => {
  const now = Date.now();
  if (
    adminDataLastResult &&
    now - adminDataLastAt < ADMIN_DATA_DEDUPE_WINDOW_MS
  ) {
    return adminDataLastResult;
  }

  if (!adminDataInFlight) {
    adminDataInFlight = (async () => {
      ...
      const payload = { dashboardData, complaintsMap };
      adminDataLastResult = payload;
      adminDataLastAt = Date.now();
      return payload;
    })().finally(() => {
...
const AdminDataManagement = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const { dashboardData, complaintsMap } = await fetchAdminDataPayload();
